...
<div class="container">
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $Heure_départ = $_POST["Heure_départ"];
        ...
        $arrivée_port = "%{$arrivée_port}%";
        $params[] = $depart_port;
        $params[] = $arrivée_port;

        if (!empty($date_départ)) {
            $sql .= " AND date_départ = ?";
            $types .= "s";
            $params[] = $date_départ;
        }

        if (!empty($Heure_départ)) {
            $sql .= " AND Heure_départ = ?";
            $types .= "s";
            $params[] = $Heure_départ;
        }
    ...
    ?>
    <a class="back-link" href="voyages.html">Retour</a>
</div>
</body>
</html>
